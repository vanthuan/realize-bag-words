package jp.ac.jaist.realizer.evaluation;
...
import jp.ac.jaist.srealizer.algorithms.mert.EvaluationMetric;
...
public class EvalTool {
	...
	static List<List<String>> refSentences;
	...
	public static StringBuffer evaluateCands_nbest(String inFileName, int testIndex) {
		return evaluate(candFileName, "nbest", -1, testIndex);
	}
	...
	private static StringBuffer evaluate(String inFileName, String inFileFormat, int candPerSen, int testIndex) {
		...
		List<String> topCand_str = new ArrayList<String>();
		...
		try {
			...
			if (inFileFormat.equals("plain")) {
				...
				for (int i = 0; i < numSentences; ++i) {
					...
					topCand_str.add(normalize(candidate_str, textNormMethod));
...
*/
					...
					String[] s =  line.split("[|]+");
					int read_i = Integer.parseInt(s[0]);
					if (read_i == i) {
						
						topCand_str.add(normalize(s[1], textNormMethod));
						if (i < numSentences-1) {
							while (read_i == i) {
								line = inFile.readLine();
								 s =  line.split("[|]+");
								read_i = Integer.parseInt(s[0]);
							}
						}
						n = 1;
						i += 1;
		...
		return evalMetric.printDetailedScore_fromStats(totStats_corpus,false);
	...
	public static void processArgsAndInitialize(String[] args) {
		EvaluationMetric.set_knownMetrics();
		
		// set default values
		candFileName = "cand_database_test.txt";
		candFileFormat = "nbest";
		candRank = 1;
		refFileName = "ref_database_test.txt";
		refsPerSen = 1;
		textNormMethod = 1;
		metricName = "BLEU";
