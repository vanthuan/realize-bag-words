package jp.ac.jaist.sbagorder.algorithms.mert;
...
//  protected HashMap[][] maxNgramCounts;
  ...
  protected List<List<Integer>> refWordCount;
  ...
  protected void set_maxNgramCounts()
  {
    ...
    int oldCount = 0, nextCount = 0;
      ...
      for (int i = 0; i < numSentences; ++i) {
        maxNgramCounts[i] = getNgramCountsAll(refSentences.get(i).get(0));
        ...
        for (int r = 1; r < refSentences.get(i).size(); ++r) {
          HashMap<String,Integer> nextNgramCounts = getNgramCountsAll(refSentences.get(i).get(r));
    ...
    // For efficiency, calculate the reference lenghts, which will be used in effLength...
      ...
      refWordCount = new ArrayList<List<Integer>>();
      for (int i = 0; i < numSentences; ++i) {
      	if(refWordCount.size() > i) refWordCount.set(i,new  ArrayList<Integer>());
      	else refWordCount.add(new ArrayList<Integer>());
        for (int r = 0; r < refSentences.get(i).size(); ++r) {
          refWordCount.get(i).add(wordCount(refSentences.get(i).get(r)));
        }
      }
...
//          maxRefGramCount = maxNgramCounts[i][n].get(gram);
  ...
  public int effLength(int candLength, int i)
  {
    if (effLengthMethod == EffectiveLengthMethod.CLOSEST) { // closest
      ...
      int closestRefLength = refWordCount.get(i).get(0);
      int minDiff = Math.abs(candLength-closestRefLength);

      for (int r = 1; r < refWordCount.get(i).size(); ++r) {
        int nextRefLength = refWordCount.get(i).get(r);
      ...
      int shortestRefLength = refWordCount.get(i).get(0);
...
*/
  ...
  public StringBuffer printDetailedScore_fromStats(int[] stats, boolean oneLiner)
  {
    ...
    for (int n = 1; n <= maxGramLength; ++n) {
      ...
      if (prec_n > 0) {
        if (totalGramCount > 0) {
          if (oneLiner) {
        	  s.append(n + "=" + f4.format(prec_n) + ", ");
          } else {
        	  s.append("BLEU_precision(" + n + ") = " + (int)correctGramCount + " / " + (int)totalGramCount + " = " + f4.format(prec_n));
        	  s.append("\n");
          }
        } else {
          if (oneLiner) {
        	  ...
        	  s.append("BLEU_precision(" + n + ") = N/A (candidate has no " + n + "-grams)").append("\n");
          }
        }
      } else {
        smooth_addition *= 0.5;
        ...
        if (oneLiner) {
        	...
        	s.append("BLEU_precision(" + n + ") = " + (int)correctGramCount + " / " + (int)totalGramCount + " ==smoothed==> " + f4.format(prec_n)).append("\n");
        }
      }
    ...
    if (oneLiner) {
    	s.append("(overall=" + f4.format(Math.exp(BLEUsum)) + "), ");
    } else {
    	s.append("BLEU_precision = " + f4.format(Math.exp(BLEUsum)));
      s.append("\n");
    }
    ...
    if (oneLiner) {
    	s.append("BP=" + f4.format(BP) + ", ");
    } else {
    	s.append("Length of candidate corpus = " + (int)c_len).append("\n");
    	s.append("Effective length of reference corpus = " + (int)r_len).append("\n");
    	s.append("BLEU_BP = " + f4.format(BP)).append("\n");
     
    }

    s.append("  => BLEU = " + f4.format(BP*Math.exp(BLEUsum))).append("\n");
    return s;
  }
