...
import jp.ac.jaist.sbagorder.properties.Properties;
...
//  private int useDisk;
  ...
  private void initialize(int randsToSkip)
  {
    ...
    List<List<String>> refSentences = new ArrayList<List<String>>();
  ...
  //  System.out.println("$$ refFileName: " + refFileName);
      ...
      while((line = inFile_refs.readLine()) != null){
    	 ...
    	 refSentences.get(count-1).add(s[1].trim());
   ...
   // normalize reference sentences
      ...
      for (int i = 0; i < numSentences; ++i) {
        for (int r = 0; r < refSentences.get(i).size(); ++r) {
         ...
         /// System.out.println("===========>$$ refSentences" + "[" + i + "][" +r + "]:  " + refSentences.get(i).get(r));

          refSentences.get(i).set(r,normalize(refSentences.get(i).get(r), textNormMethod));
        }
      }
      ...
      if (decoderCommandFileName != null) {
        if (fileExists(decoderCommandFileName)) {
          ...
          System.out.println("$$ decoderCommand: " + decoderCommand);
          inFile_comm.close();
        }
      }
...
//    HashMap<Integer,int[]>[] suffStats_array = new HashMap[numSentences];
    ...
    for (int i = 0; i < numSentences; ++i) {
//      lastUsedIndex[i] = -1;
      maxIndex[i] = Properties.getProperties().getNbestsSize().get(i) -1;
...
//    printMemoryUsage();
    println("----------------------------------------------------",1);
    println("",1);
    Properties.getProperties().setLamda(lambda);
      ...
      try {
        ...
        for (int i = 0; i < numSentences; ++i) {
          ...
          // which is not true for previous iterations.
            ...
            for (int n = 0; n <= Properties.getProperties().getNbestsSize().get(i); ++n) {
            ...
            // a complete list of sizeOfNBest candidates.
              ...
              if (sents_str.equals("||||||")) {
                n = Properties.getProperties().getNbestsSize().get(i)+1;
...
//                println("",4);
          ...
          */
          ...
          String[] sentsCurrIt_currSrcSent = new String[Properties.getProperties().getNbestsSize().get(i)+1];
...
*/
        ...
        //	System.out.println("=====> run hear");
          evalMetric.createSuffStatsFile(tmpDirPrefix+"temp.currIt.unknownCands", tmpDirPrefix+"temp.currIt.unknownIndices", tmpDirPrefix+"temp.stats.unknown");
...
*/
        ...
        // in a well formed file, we'd find the nth candidate for the ith sentence
    	 String curIndex = line.substring(0,line.indexOf("|||")).trim();
       

        if (prev_index != null && !prev_index.equals(curIndex)) {
       ...
       // System.out.println("cand: " + candidate_str);
      ...
      //  System.out.println(i);
        ++n;
        /*if (n == Properties.getProperties().getNbestsSize().get(i)) {
          writeLine("||||||",outFile_sents);
          outFile_feats.println("||||||");
          n = 0; ++i;
        }*/
    ...
    int[] maxIndex, double[] featVal)
  {
    ...
    if (k > maxIndex[i]) {
      for (int c = 1; c <= numParams; ++c) {
        ...
        featVal_array[c][i] = new double[1+maxIndex[i]+Properties.getProperties().getNbestsSize().get(i)];

        for (int k2 = 0; k2 <= maxIndex[i]; ++k2) {
          featVal_array[c][i][k2] = temp[k2];
        }
      }
      maxIndex[i] += Properties.getProperties().getNbestsSize().get(i);
