...
{
 // private static DecimalFormat f3 = new DecimalFormat("###0.00000000000");
   ...
   int N = 0;
    ...
    while (line != null) {
      if (line.startsWith("cands_file")) {
        ...
        weights[1] = Double.parseDouble((line.substring(10+1)).trim());
      } else if (line.startsWith("Headword Model")) {
        weights[2] = Double.parseDouble((line.substring(14+1)).trim());
    ...
    for (int i = 0; i < candidates.size(); ++i) {
      ...
      for (int n = 0; n < candidates.get(i).size(); ++n) {
